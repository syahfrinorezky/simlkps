...
<?= $this->section('title') ?>Dosen Industri<?= $this->endSection() ?>
...
<div class="space-y-6" x-data="{
    ...
    loading: false,
    modalOpen: false,
    modalTitle: 'Tambah Dosen Industri',
    formAction: '',
    form: {
        nama: '', nidk: '', perusahaan: '', pendidikan_tertinggi: '',
        bidang_keahlian: '', sertifikat_profesi: '', mata_kuliah_diampu: '', bobot_sks: 0
    },
    openAdd() {
        this.modalTitle = 'Tambah Dosen Industri';
        this.formAction = '<?= base_url('lecturers/industry/store') ?>';
        this.form = {
            nama: '', nidk: '', perusahaan: '', pendidikan_tertinggi: '',
            bidang_keahlian: '', sertifikat_profesi: '', mata_kuliah_diampu: '', bobot_sks: 0
        };
    ...
    openEdit(id) {
        this.modalTitle = 'Edit Dosen Industri';
        this.formAction = '<?= base_url('lecturers/industry/update') ?>/' + id;
        this.loading = true;
        this.modalOpen = true;
        fetch('<?= base_url('lecturers/industry/show') ?>/' + id)
            .then(res => res.json())
            .then(data => {
                this.form = data;
                this.loading = false;
    ...
    confirmDelete(id) {
        this.deleteUrl = '<?= base_url('lecturers/industry/delete') ?>/' + id;
...
">
    ...
    <div class="flex flex-row items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Dosen Industri</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5 hidden sm:block">Data Dosen dari Industri / Praktisi</p>
        </div>
    ...
    <!-- Stats -->
    <div class="flex overflow-x-auto sm:grid sm:grid-cols-2 lg:grid-cols-2 gap-3 sm:gap-4 pb-2 sm:pb-0 snap-x hide-scrollbar ">
        <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm min-w-[160px] sm:min-w-0 snap-start shrink-0">
            <p class="text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-3">Total Dosen Industri</p>
            <p class="text-xl sm:text-3xl font-bold text-slate-800"><?= $stats['total'] ?></p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm min-w-[160px] sm:min-w-0 snap-start shrink-0">
            <p class="text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-3">Total Bobot SKS</p>
            <p class="text-xl sm:text-3xl font-bold text-slate-800"><?= number_format($stats['total_sks'], 1) ?></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-8 sm:p-12 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <div class="space-y-1">
                ...
                <p class="text-sm text-slate-500">Tidak ada data dosen industri.</p>
            </div>
        </div>
        <?php else: ?>
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        ...
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Nama / NIDK</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Perusahaan</th>
                        ...
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">MK Diampu</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">SKS</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="p-4 font-semibold text-slate-800">
                            <?= esc($lec['nama']) ?>
                            <div class="text-xs font-normal text-slate-400 mt-0.5">NIDK: <?= esc($lec['nidk'] ?? '-') ?></div>
                        </td>
                        <td class="p-4 text-slate-600"><?= esc($lec['perusahaan'] ?? '-') ?></td>
                        <td class="p-4 text-slate-600 text-xs"><?= esc($lec['pendidikan_tertinggi'] ?? '-') ?></td>
                        <td class="p-4 text-slate-600"><?= esc($lec['bidang_keahlian'] ?? '-') ?></td>
                        <td class="p-4 text-slate-600 text-xs max-w-[150px] truncate" title="<?= esc($lec['mata_kuliah_diampu'] ?? '') ?>"><?= esc($lec['mata_kuliah_diampu'] ?? '-') ?></td>
                        <td class="p-4 text-center font-semibold text-slate-700"><?= $lec['bobot_sks'] ?></td>
        ...
        <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="relative w-full max-w-xl bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100/80 z-10">
            ...
            <form :action="formAction" method="POST" class="space-y-0">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-slate-900 tracking-tight" x-text="modalTitle">Tambah Dosen Industri</h3>
                ...
                <div class="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div>
                            <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">NIDK</label>
                            <input type="text" name="nidk" x-model="form.nidk" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Perusahaan / Industri</label>
                            <input type="text" name="perusahaan" x-model="form.perusahaan" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                        </div>
                        <div>
                            <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Pendidikan Tertinggi</label>
                            <input type="text" name="pendidikan_tertinggi" x-model="form.pendidikan_tertinggi" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all" placeholder="Contoh: S2, S3">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Bobot Kredit SKS</label>
                            <input type="number" name="bobot_sks" x-model="form.bobot_sks" step="any" min="0"  class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Sertifikat Profesi</label>
                        <input type="text" name="sertifikat_profesi" x-model="form.sertifikat_profesi" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                    </div>
        ...
        <div class="bg-white rounded-2xl max-w-sm w-full overflow-hidden shadow-xl border border-slate-200" @click.outside="deleteOpen = false">
            <div class="p-6 text-center space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-slate-500">Apakah Anda yakin ingin menghapus data dosen industri ini? Tindakan ini tidak dapat dibatalkan.</p>
                </div>
            </div>
