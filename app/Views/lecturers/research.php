...
<div class="space-y-6" x-data="{
    ...
    loading: false,
    modalOpen: false,
    modalTitle: 'Tambah Penelitian',
    formAction: '',
    form: {
        lecturer_name: '', judul_penelitian: '', tema_roadmap: '',
        sumber_dana: 'pt_mandiri', jumlah_dana: 0,
        tahun: new Date().getFullYear()
    },
    openAdd() {
        this.modalTitle = 'Tambah Penelitian';
        this.formAction = '<?= base_url('lecturers/research-performance/store') ?>';
        this.form = {
            lecturer_name: '', judul_penelitian: '', tema_roadmap: '',
            sumber_dana: 'pt_mandiri', jumlah_dana: 0,
            tahun: new Date().getFullYear()
    ...
    openEdit(id) {
        this.modalTitle = 'Edit Penelitian';
        this.formAction = '<?= base_url('lecturers/research-performance/update') ?>/' + id;
        this.loading = true;
        this.modalOpen = true;
        fetch('<?= base_url('lecturers/research-performance/show') ?>/' + id)
            .then(res => res.json())
            .then(data => {
    ...
    confirmDelete(id) {
        this.deleteUrl = '<?= base_url('lecturers/research-performance/delete') ?>/' + id;
...
">
    ...
    <div class="flex flex-row items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Penelitian DTPS</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5 hidden sm:block">Penelitian Dosen Tetap Program Studi</p>
        </div>
    ...
    <!-- TABEL 1: RANGKUMAN AGREGASI PENELITIAN -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/50">
            <h3 class="font-bold text-slate-800 text-sm">Rangkuman Jumlah Judul Penelitian DTPS</h3>
        </div>
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50/70 border-b border-slate-200 text-slate-600 font-semibold">
                        ...
                        <th class="p-4 text-center border-l border-slate-200 whitespace-nowrap" colspan="3">Jumlah Judul Penelitian</th>
    ...
    <!-- TABEL 2: DETAIL DATA PENELITIAN -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <h3 class="font-bold text-slate-800 text-sm">Daftar Detail Data Penelitian</h3>
            <form method="GET" action="<?= base_url('lecturers/research-performance') ?>" class="flex gap-2">
        ...
        <div class="p-8 sm:p-12 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <div class="space-y-1">
                ...
                <p class="text-sm text-slate-500">Tidak ada detail data penelitian untuk periode ini.</p>
            </div>
        </div>
        <?php else: ?>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        ...
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider whitespace-nowrap">Judul Penelitian</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($researches as $i => $r): ?>
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="p-4 text-center text-slate-400"><?= $i + 1 ?></td>
                        <td class="p-4 font-semibold text-slate-800"><?= esc($r['nama'] ?? '-') ?></td>
                        <td class="p-4">
                            <p class="font-medium text-slate-700 leading-tight"><?= esc($r['judul_penelitian']) ?></p>
                            <p class="text-xs text-slate-400 mt-0.5">Roadmap: <?= esc($r['tema_roadmap'] ?? '-') ?></p>
                        </td>
                        <td class="p-4">
                            <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">
                                <?= ['pt_mandiri' => 'PT Mandiri', 'lembaga_dalam_negeri' => 'Dalam Negeri', 'lembaga_luar_negeri' => 'Luar Negeri'][$r['sumber_dana']] ?? $r['sumber_dana'] ?>
                            </span>
                        </td>
                        <td class="p-4 text-right font-semibold text-slate-700"><?= $r['jumlah_dana'] ? number_format($r['jumlah_dana'], 0, ',', '.') : '-' ?></td>
        ...
        <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="relative w-full max-w-xl bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100/80 z-10">
            ...
            <form :action="formAction" method="POST" class="space-y-0">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-slate-900 tracking-tight" x-text="modalTitle">Tambah Penelitian</h3>
                ...
                <div class="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div>
                        ...
                        <input type="text" name="lecturer_name" x-model="form.lecturer_name" required placeholder="Ketik nama dosen peneliti" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                    </div>
                    <div>
                        <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Judul Penelitian *</label>
                        <textarea name="judul_penelitian" x-model="form.judul_penelitian" required rows="3" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all resize-none"></textarea>
                    </div>
                    <div>
                        <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Tema / Roadmap Penelitian</label>
                        <input type="text" name="tema_roadmap" x-model="form.tema_roadmap" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                    </div>
        ...
        <div class="bg-white rounded-2xl max-w-sm w-full overflow-hidden shadow-xl border border-slate-200" @click.outside="deleteOpen = false">
            <div class="p-6 text-center space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-slate-500">Apakah Anda yakin ingin menghapus data penelitian ini? Tindakan ini tidak dapat dibatalkan.</p>
                </div>
            </div>
