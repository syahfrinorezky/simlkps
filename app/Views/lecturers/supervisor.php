...
<?= $this->section('title') ?>Pembimbing Tugas Akhir<?= $this->endSection() ?>
...
<div class="space-y-6" x-data="{
    ...
    loading: false,
    modalOpen: false,
    modalTitle: 'Tambah Bimbingan Tugas Akhir',
    formAction: '',
    form: {
        lecturer_name: '',
        bimbingan_ps_ts2: 0, bimbingan_ps_ts1: 0, bimbingan_ps_ts: 0,
        bimbingan_ps_lain_ts2: 0, bimbingan_ps_lain_ts1: 0, bimbingan_ps_lain_ts: 0
    },
    openAdd() {
        this.modalTitle = 'Tambah Bimbingan Tugas Akhir';
        this.formAction = '<?= base_url('lecturers/supervisor/store') ?>';
        this.form = {
            lecturer_name: '',
            bimbingan_ps_ts2: 0, bimbingan_ps_ts1: 0, bimbingan_ps_ts: 0,
            bimbingan_ps_lain_ts2: 0, bimbingan_ps_lain_ts1: 0, bimbingan_ps_lain_ts: 0
    ...
    openEdit(id) {
        this.modalTitle = 'Edit Bimbingan Tugas Akhir';
        this.formAction = '<?= base_url('lecturers/supervisor/update') ?>/' + id;
        this.loading = true;
        this.modalOpen = true;
        fetch('<?= base_url('lecturers/supervisor/show') ?>/' + id)
            .then(res => res.json())
            .then(data => {
                // Map lecturer name from object to local form field
    ...
    confirmDelete(id) {
        this.deleteUrl = '<?= base_url('lecturers/supervisor/delete') ?>/' + id;
...
">
    ...
    <div class="flex flex-row items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Pembimbing Tugas Akhir</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5 hidden sm:block">Data Bimbingan Dosen per Periode</p>
        </div>
    ...
    <div class="flex overflow-x-auto sm:grid gap-3 sm:gap-4 pb-2 sm:pb-0 snap-x hide-scrollbar sm:sm:grid-cols-2 lg:grid-cols-4 sm: pb-2 sm:pb-0 snap-x">
        <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm min-w-[160px] sm:min-w-0 snap-start shrink-0">
            <p class="text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-3">Dosen Pembimbing</p>
            <p class="text-xl sm:text-3xl font-bold text-slate-800"><?= $stats['total_dosen'] ?></p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm min-w-[160px] sm:min-w-0 snap-start shrink-0">
            <p class="text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-3">Bimbingan PS (TS-2)</p>
            <p class="text-xl sm:text-3xl font-bold text-slate-800"><?= $stats['total_ps_ts2'] ?></p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm min-w-[160px] sm:min-w-0 snap-start shrink-0">
            <p class="text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-3">Bimbingan PS (TS-1)</p>
            <p class="text-xl sm:text-3xl font-bold text-slate-800"><?= $stats['total_ps_ts1'] ?></p>
        </div>
        <div class="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 shadow-sm min-w-[160px] sm:min-w-0 snap-start shrink-0">
            <p class="text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-3">Bimbingan PS (TS)</p>
            <p class="text-xl sm:text-3xl font-bold text-slate-800"><?= $stats['total_ps_ts'] ?></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 sm:p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        <form method="GET" action="<?= base_url('lecturers/supervisor') ?>" class="grid grid-cols-2 sm:flex sm:flex-row gap-3 w-full">
            <select name="period_id" onchange="this.form.submit()" class="col-span-1 sm:w-auto px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary bg-slate-50 text-slate-700 transition-all">
                ...
                <option value="<?= $p['id'] ?>" <?= $period_id == $p['id'] ? 'selected' : '' ?>>
                    <?= esc($p['nama_periode']) ?> (<?= $p['tahun_akademik'] ?>)
                </option>
                <?php endforeach; ?>
            </select>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-8 sm:p-12 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <div class="space-y-1">
                ...
                <p class="text-sm text-slate-500">Tidak ada data bimbingan tugas akhir.</p>
            </div>
        </div>
        <?php else: ?>
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200">
                        ...
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">PS TS-2</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">PS TS-1</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">PS TS</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">PS Lain TS-2</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">PS Lain TS-1</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center whitespace-nowrap">PS Lain TS</th>
                        <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-24 text-center whitespace-nowrap">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($supervisions as $i => $s): ?>
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="p-4 text-center text-slate-400"><?= $i + 1 ?></td>
                        <td class="p-4 font-semibold text-slate-800">
                            <?= esc($s['nama']) ?>
                            <div class="text-xs font-normal text-slate-400 mt-0.5"><?= esc($s['nidn'] ?? '-') ?></div>
                        </td>
                        <td class="p-4 text-center font-semibold text-slate-700"><?= $s['bimbingan_ps_ts2'] ?></td>
                        <td class="p-4 text-center font-semibold text-slate-700"><?= $s['bimbingan_ps_ts1'] ?></td>
                        <td class="p-4 text-center font-semibold text-slate-700"><?= $s['bimbingan_ps_ts'] ?></td>
                        <td class="p-4 text-center text-slate-500"><?= $s['bimbingan_ps_lain_ts2'] ?></td>
                        <td class="p-4 text-center text-slate-500"><?= $s['bimbingan_ps_lain_ts1'] ?></td>
                        <td class="p-4 text-center text-slate-500"><?= $s['bimbingan_ps_lain_ts'] ?></td>
        ...
        <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-4" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="relative w-full max-w-xl bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100/80 z-10">
            ...
            <form :action="formAction" method="POST" class="space-y-0">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-slate-900 tracking-tight" x-text="modalTitle">Tambah Bimbingan</h3>
                ...
                <div class="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div>
                        <label class="block text-[10px] sm:text-[10px] sm:text-xs font-semibold text-slate-500 uppercase truncate tracking-wider mb-2">Nama Lengkap Dosen Pembimbing *</label>
                        <input type="text" name="lecturer_name" x-model="form.lecturer_name" required placeholder="Masukkan nama dosen pembimbing" class="w-full px-3 py-2 sm:px-4 sm:py-3 bg-slate-50/50 text-sm border border-slate-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all">
                    </div>

                    <div class="border-t border-slate-100 pt-4">
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Jumlah Mahasiswa Bimbingan Program Studi Ini</p>
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                <label class="block text-xs text-slate-600 mb-1">TS-2</label>
                                <input step="any" type="number" name="bimbingan_ps_ts2" x-model="form.bimbingan_ps_ts2" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none text-center">
                            </div>
                            <div>
                                <label class="block text-xs text-slate-600 mb-1">TS-1</label>
                                <input step="any" type="number" name="bimbingan_ps_ts1" x-model="form.bimbingan_ps_ts1" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none text-center">
                            </div>
                            <div>
                                <label class="block text-xs text-slate-600 mb-1">TS</label>
                                <input step="any" type="number" name="bimbingan_ps_ts" x-model="form.bimbingan_ps_ts" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none text-center">
                            </div>
                        </div>
                    </div>

                    <div class="border-t border-slate-100 pt-4">
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Jumlah Mahasiswa Bimbingan Program Studi Lain</p>
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                <label class="block text-xs text-slate-600 mb-1">TS-2</label>
                                <input step="any" type="number" name="bimbingan_ps_lain_ts2" x-model="form.bimbingan_ps_lain_ts2" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none text-center">
                            </div>
                            <div>
                                <label class="block text-xs text-slate-600 mb-1">TS-1</label>
                                <input step="any" type="number" name="bimbingan_ps_lain_ts1" x-model="form.bimbingan_ps_lain_ts1" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none text-center">
                            </div>
                            <div>
                                <label class="block text-xs text-slate-600 mb-1">TS</label>
                                <input step="any" type="number" name="bimbingan_ps_lain_ts" x-model="form.bimbingan_ps_lain_ts" min="0" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none text-center">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl max-w-sm w-full overflow-hidden shadow-xl border border-slate-200" @click.outside="deleteOpen = false">
            <div class="p-6 text-center space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-slate-500">Apakah Anda yakin ingin menghapus data bimbingan dosen ini? Tindakan ini tidak dapat dibatalkan.</p>
                </div>
            </div>
