...
<div class="space-y-6">
    ...
    <div class="flex flex-row items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5 hidden sm:block">Jumlah Judul Publikasi Ilmiah Dosen Tetap Program Studi</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <h3 class="font-bold text-slate-800 text-sm">Input Jumlah Judul Publikasi Ilmiah DTPS</h3>
        </div>
        
        <form action="<?= base_url('lecturers/publications/store') ?>" method="POST"
            x-data="{
                ...
                <table class="w-full text-left border-collapse text-sm">
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-200 text-slate-600 font-semibold">
                            ...
                            <th class="p-4 whitespace-nowrap">Jenis Publikasi</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        ...
                        <tr>
                            ...
                            <td class="p-3 border-l border-slate-100 bg-slate-50/30">
                                <input type="number" name="summary[<?= $key ?>][ts2]" x-model.number="summary['<?= $key ?>'].ts2" min="0" required
                            ...
                            <td class="p-3 border-l border-slate-100 bg-slate-50/30">
                                <input type="number" name="summary[<?= $key ?>][ts1]" x-model.number="summary['<?= $key ?>'].ts1" min="0" required
                                    class="w-full px-3 py-1.5 bg-white border border-slate-250 rounded-xl text-center text-sm font-semibold focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary">
                            </td>
                            <td class="p-3 border-l border-slate-100 bg-slate-50/30">
                                <input type="number" name="summary[<?= $key ?>][ts]" x-model.number="summary['<?= $key ?>'].ts" min="0" required
                                    class="w-full px-3 py-1.5 bg-white border border-slate-250 rounded-xl text-center text-sm font-semibold focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary">
                            </td>
                            <td class="p-3 border-l border-slate-200 bg-slate-50/70 text-center font-bold text-slate-800"
                                x-text="(summary['<?= $key ?>'].ts2 || 0) + (summary['<?= $key ?>'].ts1 || 0) + (summary['<?= $key ?>'].ts || 0)">
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        ...
                        <tr class="bg-slate-100/60 font-bold border-t-2 border-slate-200">
                            ...
                            <td class="p-4 text-center border-l border-slate-200 text-slate-800"
                                x-text="Object.values(summary).reduce((sum, item) => sum + (parseInt(item.ts2) || 0), 0)">
                            </td>
                            <td class="p-4 text-center border-l border-slate-200 text-slate-800"
                                x-text="Object.values(summary).reduce((sum, item) => sum + (parseInt(item.ts1) || 0), 0)">
                            </td>
                            <td class="p-4 text-center border-l border-slate-200 text-slate-800"
                                x-text="Object.values(summary).reduce((sum, item) => sum + (parseInt(item.ts) || 0), 0)">
                            </td>
                            <td class="p-4 text-center border-l border-slate-200 bg-primary/5 text-primary text-base font-extrabold"
                                x-text="Object.values(summary).reduce((sum, item) => sum + (parseInt(item.ts2) || 0) + (parseInt(item.ts1) || 0) + (parseInt(item.ts) || 0), 0)">
                            </td>
                        </tr>
                    </tbody>
                </table>
