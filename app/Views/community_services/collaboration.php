...
<div class="space-y-6" x-data="{
    ...
    confirmDelete(id) {
        const deleteUrl = '<?= base_url('community-services/collaboration/delete') ?>/' + id;
        this.$dispatch('open-delete-modal', {
            url: deleteUrl,
            title: 'Hapus Data PkM',
            message: 'Apakah Anda yakin ingin menghapus data PkM kolaborasi ini?'
        });
    },
    openAdd() {
        this.modalTitle = 'Tambah Data PkM';
        this.formAction = '<?= base_url('community-services/collaboration/store') ?>';
    ...
    openEdit(id) {
        this.modalTitle = 'Edit Data PkM';
        this.formAction = '<?= base_url('community-services/collaboration/update') ?>/' + id;
        this.loading = true;
        this.modalOpen = true;
        fetch('<?= base_url('community-services/collaboration/show') ?>/' + id)
            .then(res => res.json())
            .then(data => {
...
}" x-init="totalRows = <?= count($collaborations) ?>">

    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800"><?= esc($title) ?></h2>
            <p class="text-sm text-slate-500">Kelola data kegiatan pengabdian kepada masyarakat (PkM) dosen tetap program studi (DTPS) yang melibatkan mahasiswa program studi.</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <form action="<?= base_url('community-services/collaboration') ?>" method="get" class="flex flex-col sm:flex-row items-center gap-3 w-full">
            ...
            <div class="w-full sm:flex-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?= esc($search) ?>" placeholder="Cari berdasarkan nama dosen, mahasiswa, atau judul PkM..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all">
                </div>
            </div>
        </form>
    </div>
        ...
        <?php if (empty($collaborations)) : ?>
            <div class="p-12 text-center flex flex-col items-center justify-center space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-slate-500 max-w-sm">Data kolaborasi PkM untuk kriteria yang dicari tidak ditemukan.</p>
                </div>
            </div>
        <?php else : ?>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200">
                            ...
                            <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Tema PkM sesuai Roadmap</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $no = 0; foreach ($collaborations as $c) : ?>
                            <tr class="hover:bg-slate-50/50 transition-all text-sm" data-row-idx="<?= $no ?>" x-show="isRowVisible(<?= $no ?>)">
                                ...
                                <td class="p-4 font-bold text-slate-800"><?= esc($c['nama_dosen']) ?></td>
                                <td class="p-4 text-slate-600 leading-relaxed"><?= esc($c['tema_roadmap']) ?></td>
                                <td class="p-4 font-semibold text-slate-700"><?= esc($c['nama_mahasiswa']) ?></td>
                                <td class="p-4 text-slate-600 leading-relaxed font-medium"><?= esc($c['judul_kegiatan']) ?></td>
                                <td class="p-4 text-center font-mono font-bold text-slate-700"><?= $c['tahun'] ?></td>
                                <?php if ($canModify) : ?>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 min-h-screen" x-cloak>
        ...
        <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-10">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h3 class="font-bold text-slate-800 text-lg" x-text="modalTitle">Tambah Data PkM</h3>
            ...
            <form :action="formAction" method="POST" class="p-6 space-y-4">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-xs font-semibold text-slate-600">Tema PkM Sesuai Roadmap</label>
                    <input type="text" name="tema_roadmap" x-model="temaRoadmap" required placeholder="Tema roadmap PkM..." class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary">
                </div>

                <div class="flex flex-col gap-1">
                    <label class="text-xs font-semibold text-slate-600">Judul Kegiatan PkM</label>
                    <textarea name="judul_kegiatan" x-model="judulKegiatan" required placeholder="Judul kegiatan PkM..." rows="3" class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"></textarea>
                </div>
