...
<div class="space-y-6" x-data="{
    loading: false,
    modalOpen: false,
    modalTitle: 'Tambah Data Rujukan',
    formAction: '',
    id: '',
    ...
    confirmDelete(id) {
        const deleteUrl = '<?= base_url('researches/references/delete') ?>/' + id;
        this.$dispatch('open-delete-modal', {
            url: deleteUrl,
            title: 'Hapus Data Rujukan',
            message: 'Apakah Anda yakin ingin menghapus data rujukan tesis ini?'
        });
    },
    openAdd() {
        this.modalTitle = 'Tambah Data Rujukan';
        this.formAction = '<?= base_url('researches/references/store') ?>';
    ...
    openEdit(id) {
        this.modalTitle = 'Edit Data Rujukan';
        this.formAction = '<?= base_url('researches/references/update') ?>/' + id;
        this.loading = true;
        this.modalOpen = true;
        fetch('<?= base_url('researches/references/show') ?>/' + id)
            .then(res => res.json())
            .then(data => {
                ...
                this.judulTesisDisertasi = data.judul_tesis_disertasi;
...
}" x-init="totalRows = <?= count($references) ?>">

    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800"><?= esc($title) ?></h2>
            <p class="text-sm text-slate-500">Kelola data rujukan penelitian dosen tetap program studi (DTPS) yang menjadi tema tesis/disertasi mahasiswa.</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <form action="<?= base_url('researches/references') ?>" method="get" class="flex flex-col sm:flex-row items-center gap-3 w-full">
            ...
            <div class="w-full sm:flex-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?= esc($search) ?>" placeholder="Cari berdasarkan nama dosen, mahasiswa, atau judul tesis..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all">
                </div>
            </div>
        </form>
    </div>
        ...
        <?php if (empty($references)) : ?>
            <div class="p-12 text-center flex flex-col items-center justify-center space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-slate-500 max-w-sm">Data rujukan tesis untuk kriteria yang dicari tidak ditemukan.</p>
                </div>
            </div>
        <?php else : ?>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200">
                            ...
                            <th class="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Judul Tesis/Disertasi</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php $no = 0; foreach ($references as $r) : ?>
                            <tr class="hover:bg-slate-50/50 transition-all text-sm" data-row-idx="<?= $no ?>" x-show="isRowVisible(<?= $no ?>)">
                                ...
                                <td class="p-4 text-slate-600 leading-relaxed font-medium"><?= esc($r['judul_tesis_disertasi']) ?></td>
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 min-h-screen" x-cloak>
        ...
        <div x-show="modalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-10">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h3 class="font-bold text-slate-800 text-lg" x-text="modalTitle">Tambah Data Rujukan</h3>
            ...
            <form :action="formAction" method="POST" class="p-6 space-y-4">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-xs font-semibold text-slate-600">Judul Tesis/Disertasi</label>
                    <textarea name="judul_tesis_disertasi" x-model="judulTesisDisertasi" required placeholder="Judul tesis..." rows="3" class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"></textarea>
                </div>
